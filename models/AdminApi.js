...
var AdminApi = {
  ...
  updatePatientName: function (data, callback) {
    qry = " UPDATE `tbl_patient` SET   `Name` = '" + data.PatientName + "'  where  `PatientID` = '" + data.PatientID + "'";
    return db.query(qry, callback);
  },
  ...
  getPlanByID: function (id, callback) {
    return db.query("select * from tbl_plan where PatientID = ? ", [id], callback);
  },
